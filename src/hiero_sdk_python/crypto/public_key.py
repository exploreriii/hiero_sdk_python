...
class PublicKey:
    ...
    def from_bytes_ed25519(cls, pub: bytes) -> "PublicKey":
        ...
        This loader treats the 32-byte input as an Ed25519 public key point.

        *Note* PublicKey.from_bytes_ed25519(pub_bytes) is explicitly 
        requesting raw Ed25519 public bytes.

        *Warning*: Raw Ed25519 private seeds are also 32 bytes and cannot be
        distinguished, so incorrectly passed seeds will be accepted as public bytes.
        ...
        """
        # Emit a warning to let callers know about the ambiguity:
        warnings.warn(
            "from_bytes_ed25519() cannot distinguish Ed25519 private seeds "
            "from public-key bytes; ensure you really have a public key.",
            UserWarning,
            stacklevel=2
        )
        ...
        # *Watch out!* Incorrectly passed private Ed25519 keys will be treated as public.
        try:
    ...
    #
    # ---------------------------------
    # General (Ed25519, ECDSA secp256k1, DER) from bytes loader.
    # A catch-all method that can be valuable if used correctly.
    # ---------------------------------
    #
    ...
    def from_bytes(cls, pub: bytes) -> "PublicKey":
        """
        Catch-all method to load public keys from bytes.
        Allows for Ed25519, ECDSA (secp256k1) and DER encoded.

        *Warning*: this is designed for public bytes not private bytes.
        Incorrectly passed Ed25519 private seeds will still be processed as public keys as they are indistinguishable.
        Incorrectly passed ECDSA private seeds or DER econded will be rejected by their respective libraries.
        ...
        """
        # Warn users about the ambiguity up front
        warnings.warn(
            "from_bytes() cannot distinguish Ed25519 private seeds "
            "from public-key bytes; if using Ed25519 ensure you really have a Ed25519 public key.",
            UserWarning,
            stacklevel=2
        )
        # A1) Enforce exact length for raw Ed25519 keys which are 32 bytes.
        if len(pub) == 32:
            # A2) Delegate to ed25519 cryptography library for curve-point validation
            # *Watch out!* Incorrectly passed private Ed25519 keys will be treated as public.
            try:
                ed_pub = ed25519.Ed25519PublicKey.from_public_bytes(pub)
                # A3) Return the validated ed25519 public key
                return cls(ed_pub)
            except Exception:
                # Error raised if bytes do not form a valid Ed25519 public point
                raise ValueError("Invalid 32-byte public key (not Ed25519).")

        # B1) ECDSA (secp256k1) valid point lengths are either 33 (compressed) or 65 (uncompressed).
        # Incorrectly passed private-key scalars are 32 bytes so will be rejected.
        if len(pub) in (33, 65):
            try:
                # B2) Delegate to cryptography ec library for point decoding and validation
                ec_pub = ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256K1(), pub)
                # B3) Return the validated ECDSA (secp256k1) public key
                return cls(ec_pub)
            except Exception:
                # Error raised if bytes do not correspond to a valid curve point
                raise ValueError("Failed to parse ECDSA public key from raw bytes.")

        # Else assume its DER encoded and needs serializing as Ed25519 or ECDSA
        try:
            # This loader will only allow public DER formats, not private.
            maybe_pub = serialization.load_der_public_key(pub)
            # If its Ed25519, delegate to cryptography Ed25519 library for point decoding and validation
            # *Watch out!* Incorrectly DER encoded private Ed25519 keys will be treated as public.
            if isinstance(maybe_pub, ed25519.Ed25519PublicKey):
                return cls(maybe_pub)
            # If its ECDSA, delegate to cryptography ec library for point decoding and validation
            if isinstance(maybe_pub, ec.EllipticCurvePublicKey):
    ...
    def from_string_ed25519(cls, hex_str: str) -> "PublicKey":
        ...
        *Warning*: Raw Ed25519 private seeds are also 32 bytes (64 hex chars)
        and will be accepted here as if they were public keys.
        """
        # Warn the caller about the ambiguity
        warnings.warn(
            "from_string_ed25519() cannot distinguish a 32-byte Ed25519 seed "
            "from a public key; ensure you really have a public key.",
            UserWarning,
            stacklevel=2
        )
    ...
    def from_string(cls, hex_str: str) -> "PublicKey":
        ...
        *Warning*: Raw Ed25519 private seeds are also 32 bytes and will be
        ...
        """
        warnings.warn(
            "from_string() is for public-key data only; it cannot distinguish "
            "an Ed25519 private seed from a public key.",
    ...
    def to_bytes_ecdsa(self) -> bytes:
        """
        Specific name for clarity.
        Returns the ECDSA (secp256k1) public key in raw form.
        Typically 33 bytes (compressed).
        """
        return self._public_key.public_bytes(
